...
To re-trigger the enter animation when content changes (e.g. navigating between detail pages), change the `key`:

```jsx
<ViewTransition key={item.id} enter="fade-in">
...
In Next.js App Router, wrap page content with `<ViewTransition>` in the layout or page component. Use `startTransition` from React when triggering navigation programmatically:
...
function NavLink({ href, direction, children }) {
...
- Always respect `prefers-reduced-motion`: wrap animation CSS in `@media (prefers-reduced-motion: no-preference)`
- Shared element morphs and directional slides are most likely to cause motion sickness — prioritize reducing these
- Fades and simple opacity transitions are generally safe under reduced motion preferences
